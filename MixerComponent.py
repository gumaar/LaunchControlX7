...
SEND_ROWS_COUNT=2
SEND_INDEX_MOVE_SIZE=1

class ChannelStripComponent(ChannelStripComponentBase):
    send_lights = control_list(ButtonControl,
      control_count=SEND_ROWS_COUNT,
      color="Mixer.Sends",
...
class MixerComponent(MixerComponentBase):
    ...
    def set_send_controls(self, controls):
        ...
        for index, channel_strip in enumerate(self._channel_strips):
            if self.send_index is None:
                ...
                send_controls = [controls.get_button(index, i) for i in range(SEND_ROWS_COUNT)] if controls else [
                 None]
    ...
    def set_send_lights(self, lights):
        for index, channel_strip in enumerate(self._channel_strips):
            ...
            if lights is not None:
                lights.reset()
                elements = None if self.send_index is None else [lights.get_button(index, i) for i in range(SEND_ROWS_COUNT)]
    ...
    def set_pan_lights(self, lights):
        for strip, light in zip_longest(self._channel_strips, lights or []):
            # Somehow this is why lights work - not sure about channel strips or what it does inside 
            if strip is not None and strip.pan_light is not None:
    ...
    def _set_send_index(self, index):
        if index is not None:
            if index % SEND_INDEX_MOVE_SIZE > 0:
                index -= 1
    ...
    def _update_send_buttons(self):
        self.next_sends_button.enabled = self.send_index is not None and self.send_index < self.num_sends - SEND_ROWS_COUNT
    ...
    def next_sends_button(self, button):
        self.send_index = min(self.send_index + SEND_INDEX_MOVE_SIZE, self.num_sends - 1)
        self._control_surface._set_mode_1_sends()

    @prev_sends_button.pressed
    def prev_sends_button(self, button):
        self.send_index = max(self.send_index - SEND_INDEX_MOVE_SIZE, 0)
        self._control_surface._set_mode_1_sends()
    ...
    def set_solo_buttons(self, buttons):
        for strip, button in zip_longest(self._channel_strips, buttons or []):
            if button:
                button.set_on_off_values("Mixer.Crossfade.A", "Mixer.Crossfade.B")
